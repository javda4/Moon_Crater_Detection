...
2. The dataset has now been split into `k` folds, each having a list of `train` and `val` indices. We will construct a DataFrame to display these results more clearly.
    ...
    for i, (train, val) in enumerate(kfolds, start=1):
        ...
        folds_df[f"split_{i}"].loc[labels_df.iloc[val].index] = "yolov12_val"
    ```
...
4. Next, we create the directories and dataset YAML files for each split.
    ...
    save_path = Path(dataset_path / f"{datetime.date.today().isoformat()}_{ksplit}-Fold_Cross-yolov12_val")
    ...
    for split in folds_df.columns:
        ...
        (split_dir / "yolov12_val" / "images").mkdir(parents=True, exist_ok=True)
        (split_dir / "yolov12_val" / "labels").mkdir(parents=True, exist_ok=True)
...
3. You can also use [Ultralytics data.utils.autosplit](https://docs.ultralytics.com/reference/data/utils/) function for automatic dataset splitting:
    ...
    # Automatically split dataset into train/yolov12_val/test
