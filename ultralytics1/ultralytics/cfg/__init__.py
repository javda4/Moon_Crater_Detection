...
MODES = frozenset({"train", "yolov12_val", "predict", "export", "track", "benchmark"})
...
def merge_equals_args(args: List[str]) -> List[str]:
    ...
    This function handles the following cases:
        1. ['arg', '=', 'yolov12_val'] becomes ['arg=yolov12_val']
        2. ['arg=', 'yolov12_val'] becomes ['arg=yolov12_val']
        3. ['arg', '=yolov12_val'] becomes ['arg=yolov12_val']
    ...
    while i < len(args):
        ...
        if arg == "=" and 0 < i < len(args) - 1:  # merge ['arg', '=', 'yolov12_val']
            new_args[-1] += f"={args[i + 1]}"
            i += 2
            continue
        elif arg.endswith("=") and i < len(args) - 1 and "=" not in args[i + 1]:  # merge ['arg=', 'yolov12_val']
            new_args.append(f"{arg}{args[i + 1]}")
            i += 2
            continue
        elif arg.startswith("=") and i > 0:  # merge ['arg', '=yolov12_val']
            new_args[-1] += arg
            i += 1
            continue
...
def entrypoint(debug: str = "") -> None:
    ...
    Examples:
        ...
        >>> entrypoint("yolov12_val model=yolo11n.pt data=coco8.yaml batch=1 imgsz=640")
    ...
    elif mode in {"train", "yolov12_val"}:
