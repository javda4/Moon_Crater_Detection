...
def check_det_dataset(dataset, autodownload=True):
    ...
    # Checks
    for k in "train", "yolov12_val":
        if k not in data:
            if k != "yolov12_val" or "validation" not in data:
                raise SyntaxError(
                    emojis(f"{dataset} '{k}:' key missing ❌.\n'train' and 'yolov12_val' are required in all data YAMLs.")
                )
            LOGGER.info("WARNING ⚠️ renaming data YAML 'validation' key to 'yolov12_val' to match YOLO format.")
            data["yolov12_val"] = data.pop("validation")  # replace 'validation' key with 'yolov12_val' key
    ...
    for k in "train", "yolov12_val", "test", "minival":
    ...
    # Parse YAML
    val, s = (data.get(x) for x in ("yolov12_val", "download"))
    if val:
        val = [Path(x).resolve() for x in (val if isinstance(val, list) else [val])]  # yolov12_val path
...
def check_cls_dataset(dataset, split=""):
    ...
    Args:
        ...
        split (str, optional): The split of the dataset. Either 'yolov12_val', 'test', or ''.

    Returns:
        (dict): A dictionary containing the following keys:
            - 'train' (Path): The directory path containing the training set of the dataset.
            - 'yolov12_val' (Path): The directory path containing the validation set of the dataset.
    ...
    val_set = (
        data_dir / "yolov12_val"
        if (data_dir / "yolov12_val").exists()
        else data_dir / "validation"
        if (data_dir / "validation").exists()
        else None
    )  # data/test or data/yolov12_val
    test_set = data_dir / "test" if (data_dir / "test").exists() else None  # data/yolov12_val or data/test
    if split == "yolov12_val" and not val_set:
        LOGGER.warning("WARNING ⚠️ Dataset 'split=yolov12_val' not found, using 'split=test' instead.")
        val_set = test_set
    elif split == "test" and not test_set:
        LOGGER.warning("WARNING ⚠️ Dataset 'split=test' not found, using 'split=yolov12_val' instead.")
        test_set = val_set
    ...
    # Print to console
    for k, v in {"train": train_set, "yolov12_val": val_set, "test": test_set}.items():
    ...
    return {"train": train_set, "yolov12_val": val_set, "test": test_set, "nc": nc, "names": names}
...
class HUBDatasetStats:
    ...
    def get_json(self, save=False, verbose=False):
        ...
        for split in "train", "yolov12_val", "test":
...
def autosplit(path=DATASETS_DIR / "coco8/images", weights=(0.9, 0.1, 0.0), annotated_only=False):
    """
    Automatically split a dataset into train/yolov12_val/test splits and save the resulting splits into autosplit_*.txt files.
