...
class YOLOETrainer(DetectionTrainer):
    ...
    def build_dataset(self, img_path, mode="train", batch=None):
        ...
        return build_yolo_dataset(
            self.args, img_path, batch, self.data, mode=mode, rect=mode == "yolov12_val", stride=gs, multi_modal=mode == "train"
        )
...
class YOLOETrainerFromScratch(YOLOETrainer):
    ...
    def get_dataset(self):
        ...
        assert data_yaml.get("yolov12_val", False), "validation dataset not found"  # lvis.yaml
        data = {k: [check_det_dataset(d) for d in v.get("yolo_data", [])] for k, v in data_yaml.items()}
        assert len(data["yolov12_val"]) == 1, f"Only support validating on 1 dataset for now, but got {len(data['yolov12_val'])}."
        val_split = "minival" if "lvis" in data["yolov12_val"][0]["yolov12_val"] else "yolov12_val"
        for d in data["yolov12_val"]:
            if d.get("minival") is None:  # for lvis dataset
                continue
            d["minival"] = str(d["path"] / d["minival"])
        for s in ["train", "yolov12_val"]:
        ...
        final_data["nc"] = data["yolov12_val"][0]["nc"]
        final_data["names"] = data["yolov12_val"][0]["names"]
        # NOTE: add path with lvis path
        final_data["path"] = data["yolov12_val"][0]["path"]
        ...
        return final_data["train"], final_data["yolov12_val"][0]
    ...
    def final_eval(self):
        ...
        """
        val = self.args.data["yolov12_val"]["yolo_data"][0]
        self.validator.args.data = val
        self.validator.args.split = "minival" if isinstance(val, str) and "lvis" in val else "yolov12_val"
