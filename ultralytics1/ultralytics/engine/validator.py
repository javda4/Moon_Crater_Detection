...
"""
Check a model's accuracy on a test or yolov12_val split of a dataset.

Usage:
    $ yolo mode=yolov12_val model=yolo11n.pt data=coco8.yaml imgsz=640

Usage - formats:
    $ yolo mode=yolov12_val model=yolo11n.pt                 # PyTorch
...
class BaseValidator:
    ...
    def __call__(self, trainer=None, model=None):
        ...
        if self.training:
            ...
            # Force FP16 yolov12_val during training
            ...
            if self.device.type in {"cpu", "mps"}:
                self.args.workers = 0  # faster CPU yolov12_val as time dominated by inference, not dataloading
            if not pt:
        ...
        self.jdict = []  # empty before each yolov12_val
        ...
        if self.training:
            model.float()
            results = {**stats, **trainer.label_loss_items(self.loss.cpu() / len(self.dataloader), prefix="yolov12_val")}
