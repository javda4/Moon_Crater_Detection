...
class Model(torch.nn.Module):
    ...
    Methods:
        ...
        yolov12_val: Validates the model on a dataset.
    ...
    def _check_is_pytorch_model(self) -> None:
        ...
        if not (pt_module or pt_str):
            raise TypeError(
                ...
                f"PyTorch models can train, yolov12_val, predict and export, i.e. 'model.train(data=...)', but exported "
                f"formats like ONNX, TensorRT etc. only support 'predict' and 'yolov12_val' modes, "
    ...
    def val(
        ...
        **kwargs: Any,
        ...
        Examples:
            >>> model = YOLO("yolo11n.pt")
            >>> results = model.yolov12_val(data="coco8.yaml", imgsz=640)
            >>> print(results.box.map)  # Print mAP50-95
        """
        custom = {"rect": True}  # method defaults
        args = {**self.overrides, **custom, **kwargs, "mode": "yolov12_val"}  # highest priority args on the right
